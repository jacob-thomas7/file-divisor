...
// Intentionally out of namespace to keep it private. Helper function only for translation unit.
void add_valid_files(const file_divisor::SearchTerms& search_terms, const std::filesystem::path& system_path, std::vector<std::filesystem::path>* result){
    auto file_status = std::filesystem::status(system_path);
    
    switch(file_status.type()){
        case std::filesystem::file_type::regular:
            // Filter results
            for(const file_divisor::Filter& filter : search_terms.filters){
        ...
        case std::filesystem::file_type::directory:
            ...
            for(const std::filesystem::directory_entry& directory_entry : std::filesystem::directory_iterator(system_path)){
                add_valid_files(search_terms, directory_entry.path(), result);
            }
            break;
...
namespace file_divisor {
    void SearchTerms::add_system_path(const std::filesystem::path& path){
        this->system_paths.emplace_back(path);
    }

    void SearchTerms::add_filter(const std::function<bool(const std::filesystem::path&)>& selector, FilterType type){
    ...
    std::vector<std::filesystem::path>&& search(const SearchTerms& search_terms){
        ...
        for(const std::filesystem::path& system_path : search_terms.system_paths){
            add_valid_files(search_terms, system_path, result);
        }
